...
namespace AccelBrain
{
    ...
    /// to be able to take responsibility for only monaural beat.
    /// </summary>
    public class MonauralBeat : BrainBeat
    {
        /// <summary>
        /// Updates an array of floats comprising the audio data.
        /// There is difference between the binaural beat and monaural beat.
        /// This concrete method must take responsibility for only monaural beat.
        /// </summary>
        ...
        public override float[] UpdatePhase (float[] data, int channels)
        {
            ...
            for (var i = 0; i < data.Length; i = i + channels)
            {
                ...
                data[i] += (float)(this.Gain * Math.Sin(this.RightPhase));
                if (channels > 1)
                {
                    data[i + 1] = (float)(this.Gain * Math.Sin(this.LeftPhase));
